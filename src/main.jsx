...
function App() {
  ...
  return <div className="wrap">
    ...
    <div className="results">{filtered.slice(0, 500).map((item, i) => <article key={`${item.Node}-${i}`} className={`card ${(item.Tecnologia || '').toLowerCase()} ${open === i ? 'open' : ''}`}><button className="card-head" onClick={() => setOpen(open === i ? null : i)}><div className="card-id"><div className="node"><span className="tech-dot" />NODE {highlight(item.Node, q)}</div><div className="bairro">{highlight(item.Bairro, q)}</div></div><div className="card-right"><span className={`badge ${(item.Tecnologia || '').toLowerCase()}`}>{item.Tecnologia}</span><span className="vlan-tag">VLAN {highlight(item.VLAN, q)}</span><span className="caret">▶</span></div></button><div className="card-summary-row"><span>OLT: <b>{highlight(item.OLT, q)}</b></span><span>PON: <b>{highlight(item.PON, q)}</b></span><span>Clientes: <b>{highlight(item.Qtd_Clientes, q)}</b></span></div>{open === i && <div className="card-detail"><div className="grid">{fields.map(([label, key]) => { const raw = item[key]; const masked = key === 'Senha_Gerencia' && raw && !showPass; return <div className={`field ${freeTextKeys.includes(key) ? 'obs' : ''}`} key={key}><div className="lbl">{label}</div><div className={`val ${!raw ? 'empty' : ''}`}>{!raw ? 'Não informado' : masked ? '••••••••' : highlight(raw, q)}</div></div> })}</div></div>}</article>)}</div>
